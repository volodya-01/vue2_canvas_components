...
export default {
  ...
  methods: {
    ...
    //第0圈(图片)
    drawScale0(ctx, x0, y0, R) {
      ctx.save();
      //ctx.translate(R+68, R+95);

      ctx.beginPath();
      ctx.translate(R * 1.25, R * 1.37);
      var hourNumbers = [3, 6, 9, 12];
      var hourNumbers2 = ["(15)", "(18)", "(21)", "(24)"];
      ctx.font = "18px Arial";
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.font = `${14}px MicrosoftYaHei`;
      ctx.fillStyle = "#fff"; //填充圆
      ctx.fill();
      //小时数字
      hourNumbers.forEach(function(number, i) {
        ...
        var x = Math.cos(rad) * (R * 1.15);
        var y = Math.sin(rad) * (R * 1.15);
        ctx.fillText(number, x, y);
        // console.log(x)
      });

      hourNumbers2.forEach(function(number, i) {
        var rad = ((2 * Math.PI) / 4) * i;
        //var x = Math.cos(rad) * (R + 25);
        //var y = Math.sin(rad) * (R + 25);
        var x = Math.cos(rad) * (R * 1.08);
        var y = Math.sin(rad) * (R * 1.08);
        ctx.fillText(number, x, y);
        // console.log(x)
      });
    },
    ...
    drawScale10(ctx, x0, y0, R) {
      ...
      ctx.fill();
      ctx.fillText("24", x0 * 0.91, y0 * 0.94);
      ctx.closePath();
      ctx.beginPath();
      ctx.font = `${R * 0.089}px MicrosoftYaHei`;
      ctx.fillStyle = "#6C6C6C"; //填充圆
      ctx.fill();
      ctx.fillText("报警总数", x0 * 0.86, y0 * 1.1);
      ctx.closePath();
      ctx.restore();
    }
  }
